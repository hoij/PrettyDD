...
int parse(std::string line, vector<Player>& players) {
	...
	if (!words.size() == 0) {
		...
		find_values(words, line_info);

		// Debug prints
		cout << "Dealer: " << line_info.dealer_name << "\t" << "Receiver: "<<
            line_info.receiver_name << "\t" << "Amount: " <<
			line_info.amount << "\t" << "Type: " <<
			line_info.type << "\t" << "Subtype: " << line_info.subtype << endl;

		add_to_players(players, line_info);
	}
	return 0;
}
...
int find_values(const vector<string>& words, Player::line_info& line_info) {
	...
	if (words[1] == "Other hit by other" || words[1] == "Your pet hit by other") {
		...
		line_info.dealer_name = m[0];
		regex_search(words[4], m, regex("hit\\s"		// Find "hit " then
										"(.*?)"			// match everything following, non-greedy
														// i.e. until first occurrence, of
										"(?= for)"));	// " for"
		line_info.receiver_name = m[1];
		line_info.type = "Damage";
		line_info.subtype = find_subtype(words);
		line_info.amount = find_amount(words);
		line_info.crit = is_crit(words);
	}
		...
		line_info.dealer_name = m[1];
		regex_search(words[4], m, regex("(.*?)(?= was)"));
		line_info.receiver_name = m[0];
		line_info.type = "damage";
		line_info.subtype = find_subtype(words);
		line_info.amount = find_amount(words);
		// Can this crit?
		...
		*/
        ...
        line_info.miss = true;
		line_info.type = "damage";
		line_info.subtype = find_subtype(words); // Does not work. I guess specials
		...
		line_info.miss = true;
		line_info.type = "damage";
        line_info.subtype = find_subtype(words);  // Does not find Brawl
	}
		...
		line_info.nanos.name = m[1];
		line_info.nanos.count += 1;
		cout << "Nano casted: " << line_info.nanos.name << ". Count: " << line_info.nanos.count << endl;

		// Need to check if the nano was executed succesfully or countered or fumbled or resisted
		// This info comes in the next line(s).
		// The check needs to be done when the player is know. So somewhere in the player.add() code.
	}
		...
		line_info.receiver_name = "You";
		line_info.type = "xp";
		line_info.subtype = "shadowknowledge";
		if (regex_search(words[4], m, regex("(?:gained\\s)(\\d+)")))
			line_info.amount = stoi(m[1]);
		else {
		...
		line_info.amount = stoi(xp[0]);
		if (regex_search(words[4], m, regex("(?:new\\s)(.*?)(?=\\.)"))) {
			line_info.subtype = m[1];
		}
		else {
			line_info.subtype = "xp";
		}
		line_info.amount = stoi(xp[0]);
	}
		...
		line_info.amount = stoi(m[0]);
		line_info.subtype = "research";
	}
	else
